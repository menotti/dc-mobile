...
\begin{figure}[h]
  \centering
  \includegraphics[width=.75\textwidth]{figuras/apple_navigation_interface.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=.75\textwidth]{figuras/apple_tabbar_interface.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.7]{figuras/3/tela_novo_projeto_11.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=1]{figuras/3/tela_novo_projeto_12.png}
...
\paragraph{}Vamos inicialmente adicionar um \texttt{\textbf{UILabel}} e um \texttt{\textbf{UIButton}} com textos de exemplo à tela. Arraste os objetos de forma a obter um layout parecido com o mostrado na figura \ref{fig:arrastando_itens}.
...
\paragraph{}O código deve ficar como mostrado na imagem \ref{fig:IBAction_msg}.
...
\paragraph{}O código cria um objeto do tipo \texttt{\textbf{UIAlertView}} na linha 2, já inicializando-o com alguns textos para exibição. Na linha 6 é invocado um método da classe para que o alerta seja exibido na tela. A estrutura do código é mostrada na imagem \ref{fig:IBAction_msg2}.
...
\paragraph{}Voltando à classe da primeira tela (\emph{FirstAppViewController.m}), podemos agora criar um título para a tela, que aparecerá na barra de navegação. No método \texttt{\textbf{viewDidLoad}} deixe como abaixo:
...
\paragraph{}Agora o aplicativo ainda está extremamente cru, mas já podemos executá-lo no \emph{iOS Simulator} para ver sua primeira aparência. Basta apertar o \emph{Run Button} no canto superior esquerdo. A figura \ref{fig:app_executanto1} mostra a tela do simulador com o aplicativo rodando.
...
\paragraph{}A tela do projeto deve estar como a figura \ref{fig:app_com_duas_telas}.
...
\paragraph{}Selecione uma imagem (nesse caso foi a de nome \emph{LogoDC.jpg}), agora temos a nova imagem disponível no \emph{Project Navigator}.
...
\paragraph{}Clicando no nome da imagem é possível alterar suas propriedades nesse projeto (as opções são exibidas no lado direito da tela do XCode). Altere a propriedade \emph{Type} de \emph{Relative to Group} para \emph{Relative to Project}, desse modo o caminho da imagem será relativo ao diretório onde se encontra o projeto.
...
\paragraph{}Adicionamos a imagem LogoDC.jpg contida no repositório deste turorial. Depois de adicionada,voltando à segunda tela do projeto (\emph{SeCondScreenViewController.xib}), selecionamos a \texttt{\textbf{UIImageView}} e selecionamos a imagem para ser exibida.
...
\paragraph{}Construimos a interface do aplicativo da forma como ilustrado na imagem \ref{fig:app_interface_cache}.
...
\paragraph{}Salvar e recuperar informação da estrutura \texttt{\textbf{NSUserDefaults}} é algo bem simples e direto, e lidamos com seus dados de forma parecida com um \texttt{\textbf{NSDictionary}}. Primeiro criamos uma variável de ponteiro para a estrutura no arquivo.h da interface do aplicativo.
...
\paragraph{}Adicionamos um \texttt{\textbf{IBOutlet}} chamado \emph{*textField} que está conectado ao textbox da interface, além de duas \texttt{\textbf{IBActions}}, a \emph{saveText} para o botão Salvar e \texttt{\textbf{retrieveText}} para o botão Recuperar.
...
\paragraph{}Adicione também a referência ao \emph{AppDelegate} para que seja possível utilizar o recurso de fechar o teclado após digitar o texto na textbox.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=.75\textwidth]{figuras/4/app_interface_estrutura_h.png}
  \caption{Arquivo.h da interface do aplicativo para armazenamento em cache}
  \label{fig:app_interface_estrutura_h}
\end{figure}

\paragraph{}A imagem \ref{fig:app_interface_estrutura_h} mostra a estrutura final do arquivo.h da interface.

\paragraph{}No arquivo.m da interface temos a estrutura das \texttt{\textbf{IBActions}} criadas.
...
\paragraph{}Na linha 7 inicializamos o objeto \emph{NSUserDefaults}.
...
\paragraph{}Complete a \texttt{\textbf{IBAction}} \emph{saveText} para salvar o texto da textbox em cache.
...
\paragraph{}Para esse projeto o método responsável por recuperar (\emph{retrieveText}) deve ser editado de forma a recuperar uma string da memória cache.
...
\paragraph{}A  \texttt{\textbf{IBAction}} \emph{clearBox} serve para apagar o texto da textbox e o \emph{completeBox} verifica que o texto atual é em branco, caso positivo preenche com o texto padrão definido como "Insira seu Texto".
...
\paragraph{}Um último método que deve inserido é o \emph{textFieldShouldReturn}, é utilizado para controlar quando o teclado deve ser ocultado na interface.
...
\paragraph{}A estrutura do arquivo.m da interface deve estar como ilustrado na figura .
