...
\cha{design}{Design}
\section{Listas \texttt{(ListView)}}
Listas são uma das formas mais simples de se mostrar informações ao usuário de forma simples e objetiva, e pode ser usada como um menu. A construção é simples, você pode começar mudando o layout de \texttt{RelativeLayout} para \texttt{LinearLayout}, a diferença entre eles é que o \textit{Relative} permite você posicionar elementos uns relativos aos outros enquanto que o \textit{Linear} segue uma estrutura, vertical ou horizontal, e que pode ser aninhada ou seja, você pode ter um layout \textit{Linear} horizontal dentro de uma das posições do layout vertical. Haverá um exemplo mais pra frente.
...
Agora você precisa adaptar esse layout customizado para cada item da lista, uma maneira simples é usando a classe \texttt{SimpleAdapter}.
